...
public class AuthService : IAuthService
{
    ...
    public async Task<AuthResponse> LoginAsync(LoginRequest request)
    {
        ...
        var update = Builders<UserEntity>.Update
            .Set(u => u.RefreshToken, HashToken(refreshToken))
            .Set(u => u.RefreshTokenExpiryTime, DateTime.UtcNow.AddDays(_jwt.RefreshTokenExpiryDays))
            .Set(u => u.UpdatedAt, DateTime.UtcNow);

        await _userRepo.UpdateAsync(user.Id, update);
    ...
    public async Task<AuthResponse> ExternalLoginAsync(ExternalLoginRequest request)
    {
        ...
        if (user == null)
        {
            ...
            {
                Email = email,
                PasswordHash = "", // No password for external auth users
                FirstName = firstName,
                LastName = lastName,
                Roles = roles,
            };
        ...
        else
        {
            ...
            // Sync roles from Azure AD on every login
            var updateBuilder = Builders<UserEntity>.Update
                .Set(u => u.UpdatedAt, DateTime.UtcNow);

            if (isAdmin && !user.Roles.Contains("MasterAdmin"))
            {
                updateBuilder = updateBuilder.Set(u => u.Roles, roles);
                user.Roles = roles;
                _logger.LogInformation("Promoted {Email} to MasterAdmin based on Azure AD Global Admin role", email);
            }
            ...
            await _userRepo.UpdateAsync(user.Id, updateBuilder);
        }
    ...
    public async Task<AuthResponse> RefreshTokenAsync(string refreshToken)
    {
        ...
        var update = Builders<UserEntity>.Update
            .Set(u => u.RefreshToken, HashToken(newRefreshToken))
            .Set(u => u.RefreshTokenExpiryTime, DateTime.UtcNow.AddDays(_jwt.RefreshTokenExpiryDays))
            .Set(u => u.UpdatedAt, DateTime.UtcNow);
    ...
    {
        var update = Builders<UserEntity>.Update
            .Set(u => u.RefreshToken, null as string)
            .Set(u => u.RefreshTokenExpiryTime, null as DateTime?)
            .Set(u => u.UpdatedAt, DateTime.UtcNow);
