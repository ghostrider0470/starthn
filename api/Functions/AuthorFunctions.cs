...
public class AuthorFunctions
{
    ...
    public AuthorFunctions(IUserService userService, IBlogService blogService, IUserPageTranslationRepository pageTranslationRepo, AuthHelper auth)
    {
        ...
        string slug)
    {
        ...
        if (lang != null)
        {
            var translation = await _pageTranslationRepo.GetAsync(user.Id, lang);
            if (translation != null)
            {
                if (!string.IsNullOrEmpty(translation.Bio)) response.Bio = translation.Bio;
                if (translation.PageContent != null && translation.PageContent.Count > 0)
                    response.PageContent = ProfileFunctions.ExtractPageHtml(translation.PageContent);
            }
        }
    ...
    private static AuthorResponse MapToAuthorResponse(UserEntity user, int postCount) => new()
    {
        ...
        PageContent = ProfileFunctions.ExtractPageHtml(user.PageContent),
        PostCount = postCount,
    };
}
