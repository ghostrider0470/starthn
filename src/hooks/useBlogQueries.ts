...
export function useBlogPostsPaged(lang?: string, query?: BlogQueryParams, initialData?: PaginatedBlogResponse | null) {
  // Only use SSR data as initialData if it actually contains posts,
  // otherwise let the client fetch from the API (needed for local dev where D1 is empty)
  const hasData = initialData && initialData.items.length > 0
  return useQuery({
    ...
    retry: 1,
    retryDelay: 2000,
    ...(hasData ? { initialData, initialDataUpdatedAt: Date.now() } : {}),
  })
}

export function useBlogPost(slug: string, lang?: string, initialData?: BlogPost | null) {
  return useQuery({
