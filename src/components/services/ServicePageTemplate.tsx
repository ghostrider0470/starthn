...
export function ServicePageTemplate({
  ...
  // Guard: when translations aren't loaded yet (SSR or loading state),
  // t() with returnObjects returns the key string instead of an object/array.
  const raw = (key: string) => {
    const val = t(key, { returnObjects: true })
    return typeof val === 'string' ? undefined : val
  }

  const capabilities = (raw(`${baseKey}.capabilities`) as string[] | undefined) ?? []
  const technologies = (raw(`${baseKey}.technologies`) as TechnologyItem[] | undefined) ?? []
  const breakdown = (raw(`${baseKey}.breakdown`) as ServiceBreakdownItem[] | undefined) ?? []
  const problem = raw(`${baseKey}.problem`) as ProblemSectionData | undefined
  const approach = raw(`${baseKey}.approach`) as ApproachSectionData | undefined
  const principles = raw(`${baseKey}.principles`) as PrinciplesSectionData | undefined
            ...
            {principles?.items?.map((principle, index) => (
              <div
                ...
                className={cn(
                  ...
                  index < (principles?.items?.length ?? 0) - 1 && 'border-b border-border/50',
                )}
              >
                {/* Number marker */}
