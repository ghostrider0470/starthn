...
import { stripLocalePrefix, withLocalePath } from '@/lib/i18n-utils'
import { LANGUAGES, countryCodeToEmoji } from '@/lib/languages'
import { cn } from '@/lib/utils'

/**
 * The language picker popover. Loaded on demand by LanguageSwitcher (the
 * trigger button), so Radix Popover and floating-ui stay out of the entry
 * chunk. The popover is anchored to that trigger.
 */
...
const REGION_TABS: Array<{ id: Region; label: string }> = [
  { id: 'all', label: 'All' },
...
export type LanguageSwitcherPanelProps = {
  open: boolean
...
export function LanguageSwitcherPanel({
  ...
  const switchLocale = useCallback(
    ...
    [currentLocale, i18n, onOpenChange],
  )
  ...
  return (
    <Popover open={open} onOpenChange={onOpenChange}>
      ...
      <PopoverContent
        ...
        // The trigger toggles the popover itself: a press on it must not
        // also count as an outside click (close, then reopen).
        ...
        // Focus goes back to the trigger (there is no PopoverTrigger to do it).
