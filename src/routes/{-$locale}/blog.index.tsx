...
import { ssrBlogPosts, ssrCategories, ssrTags } from '@/server/ssr-data'
...
function BlogIndexPage() {
  ...
  const { data: pagedData, isPending, isFetching } = useBlogPostsPaged(currentLocale, queryParams, loaderData?.posts)
  const { data: categories = [] } = usePublicCategories(loaderData?.categories)
  const { data: tags = [] } = usePublicTags(loaderData?.tags)

  const effectiveData = pagedData
  const posts = effectiveData?.items ?? []
  const totalCount = effectiveData?.totalCount ?? 0
  const totalPages = effectiveData?.totalPages ?? 1
